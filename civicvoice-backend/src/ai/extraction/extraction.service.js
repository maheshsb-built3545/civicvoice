...
async function extractComplaint(transcript, metadata = {}) {
  if (!transcript || typeof transcript !== 'string' || !transcript.trim()) {
    throw new AppError('EXTRACTION_FAILED', 502, 'Transcript text is required for extraction');
  }
  ...
  try {
    ...
    logger.error('Groq extraction API request failed', {
    ...
    throw new AppError('EXTRACTION_FAILED', 502, `Extraction request failed: ${err.message}`);
  } finally {
    clearTimeout(timeout);
  }
  ...
  if (!response.ok) {
    ...
    logger.error('Groq extraction API returned non-200 status', {
      timestamp: new Date().toISOString(),
      step: 'ai_extraction',
      traceId: metadata.traceId || null,
    ...
    await trackServiceFailure('ai_extraction', metadata.traceId || null).catch(() => {});
    throw new AppError('EXTRACTION_FAILED', 502, `Extraction service returned status ${response.status}`);
  }
  ...
  try {
    ...
    await trackServiceFailure('ai_extraction', metadata.traceId || null).catch(() => {});
    throw new AppError('EXTRACTION_FAILED', 502, 'Failed to parse extraction response JSON');
  }
  ...
  try {
    ...
    await trackServiceFailure('ai_extraction', metadata.traceId || null).catch(() => {});
    throw new AppError('EXTRACTION_FAILED', 502, 'Extractor failed to produce valid structured JSON output');
  }

  if (!structuredComplaint) {
    throw new AppError('EXTRACTION_FAILED', 502, 'Extraction returned empty response structure');
  }
