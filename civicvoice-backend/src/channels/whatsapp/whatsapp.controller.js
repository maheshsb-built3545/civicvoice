 ...
 * -----------------------------------------------------------------------
 * Orchestrates the WhatsApp webhook flow:
 *   verify signature → parse payload → enqueue job → 200 OK immediately
 */
...
async function receiveWebhook(req, res) {
  ...
  // Return 200 immediately after signature verification to avoid Meta retries
  res.sendStatus(200);
  ...
  try {
    ...
    logger.error('WhatsApp webhook body parse failed', { traceId: req.traceId, error: err.message });
    return;
  }
  ...
  if (messages.length === 0) {
    logger.info('WhatsApp webhook received with no actionable messages', { traceId: req.traceId });
    return;
  }
